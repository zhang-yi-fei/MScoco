...
net_g.load_state_dict(torch.load(generator_path + '_' + f'{start_from_epoch:03d}'), False)
net_d.load_state_dict(torch.load(discriminator_path + '_' + f'{start_from_epoch:03d}'), False)
...
# fixed noise and sentence vectors to see the progression
fixed_h = 4
fixed_w = 2
fixed_size = fixed_h * fixed_w
fixed_noise = get_noise_tensor(fixed_size).to(device)
fixed_caption = dataset.get_random_caption(fixed_size)
...
# log
writer.add_graph(JoinGAN2().to(device), [fixed_noise, fixed_text])
...
for e in range(start_from_epoch, end_in_epoch):
    ...
    for i, batch in enumerate(data_loader, 0):
        ...
        loss_d = ((score_fake + score_wrong) / 2 - score_right + lamb * ((gradient_norm - 1).pow(2))).mean()
        loss_d.backward()
        optimizer_d.step()
    ...
    net_g.eval()
    with torch.no_grad():
        fixed_fake = net_g(fixed_noise, fixed_text)
    net_g.train()
    ...
    for sample in range(fixed_size):
        ...
        plot_heatmap(fixed_fake[sample], skeleton, caption=fixed_caption[sample])
        plt.title(None)
        plt.xticks([])
        plt.yticks([])
