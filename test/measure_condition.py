...
dataset_train = HeatmapDataset(coco_keypoint_train, coco_caption_train, True)
...
for data in dataset_train.dataset:
    real_max_index_train.append(heatmap_to_max_index(dataset_train.get_heatmap(data, False)))
    for i in range(5):
...
for generator_path, al in zip(
        ['trained/model_generator', 'trained/tocompare/model_gan_generator', 'trained/tocompare/model_wgan_generator',
         'trained/tocompare/model_gp_generator', 'trained/tocompare/model_lp_generator'],
        ['', '_gan', '_wgan', '_gp', '_lp_lp']):
    ...
    with torch.no_grad():
        for k in range(10):
            ...
            for vector in gt_vector:
                vector = torch.tensor(vector, dtype=torch.float32, device=device).unsqueeze_(-1).unsqueeze_(
                    -1).unsqueeze_(
                    0)
                noise = get_noise_tensor(1).to(device)
                fake_max_index.append(
                    heatmap_to_max_index(np.array(net_g(noise, vector).squeeze().tolist()) * 0.5 + 0.5))
        ...
        plt.show()
        plt.savefig('conditional_measure_pose' + al)
        ...
        plt.ylabel('frequency')
        plt.show()
        plt.savefig('conditional_measure_text' + al)
